...
> **Paths (moved to va-workspace 2026-07-31).** Ledgers, inbox and the metrics extension are
> **relative to va-workspace**. Two paths are deliberately absolute because they live in the
> Obsidian vault, not here:
> - `Thinking Brain/Content/` — the 64-post content history that carries `hook_pattern` and
>   `post_urn`. `/post` stage 8 writes there; scoring matches against it.
> - `Thinking Brain/Sales/LinkedIn-LeadGen_Tracker_v5.xlsx` — bait outcomes.
>
...
2. **Content files:** `Thinking Brain/Content/YYYY-MM-DD — LinkedIn — *.md` — frontmatter holds `post_urn`, `hook_pattern`, `pillar`, `cta`.
3. **Bait log:** `Thinking Brain/Sales/LinkedIn-LeadGen_Tracker_v5.xlsx` (or whatever the current LeadGen tracker is) — for bait reply outcomes.
...
7. Compute **360Brew score** (2026 formula, v2 — DM attribution split):
   ...
   **DM attribution sources (3 channels, used together):**
   ...
   - `proxy_dms` — same `inbox/dms/` source. For DMs without a keyword match: attribute to the post published in the prior 48h if sender is not already in TB `Relationships/`. Noisy proxy, low weight.
...
11. **Write metrics_history into TB content file frontmatter.** When a content file is matched, append (don't overwrite) a `metrics_history` entry to its frontmatter so each post tracks its own performance curve over time:
   ```yaml
   metrics_history:
