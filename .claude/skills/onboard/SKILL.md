...
**Brief pass, then deep pass (one skill, two tiers — reuse, don't restart).**
- **Brief** = the do-now tier that makes the workspace usable: Step 1 identity, Step 2 ICP core (one-liner + focus areas), Step 3 offer, **Step 4a** light outreach voice, Step 5 industry + compliance, Step 6 gates, Step 7 media + brand, Step 8 lead magnets. Finish the brief pass by stating what deep items remain and that re-running `/onboard` continues them.
- **Deep** = Step 4b + Step 2c, run via the **`storytelling-strategist`** lens (`.claude/agents/storytelling-strategist.md`): the PAS/9-stage story, the structured profile, content pillars, the weekly calendar, the yearly content calendar, and the worked examples in `kk-post.md` + `kk-carousel.md`.
- **Resume:** the Step 0a progress report computes % + outstanding on every run, so a re-trigger surfaces exactly the missing steps and continues them (it never restarts). `/onboard voice` resumes the voice/story portion.
...
- **Compute % complete** across the onboarding checklist — Step 1 identity (`account-profile.md`); Step 2 ICP (`icp.md` + `positioning.md` + `engagement-targets.md`); Step 3 offer; Step 4 (`about-me.md`, `voice-profile.md`, `profile.md`, `voice.md`, `kk-post.md`, `kk-carousel.md`, plus from Step 2c `positioning.md` Content pillars + `references/content-calendar.md`); Step 5 industry + compliance (`config.json.industry` + `gates.md`, plus the operational ruleset if regulated); Step 6 gates (`config.json.gates`); Step 7 media (`config.json.media` + `references/brand-system.md`); Step 8 lead magnets (`lead-magnets.md`). An item counts DONE when its file exists and holds no remaining clean `[xxx]`. A `[xxx — needs your real X]` genuine-gap marker counts as DONE (it was reached; the owner has no material for it). Score `kk-post.md` and `kk-carousel.md` by **fill ratio** (filled formats ÷ total), not all-or-nothing, since they fill to sufficiency, not to zero placeholders.
...
**2c. Signature framework, lenses + content pillars → `references/positioning.md` ("Content pillars / Signature lenses" section).** Deep step — run via the **`storytelling-strategist`** lens. Capture the owner's own model so every writing skill sounds like them, not generic. Use the owner's exact wording wherever they give it; never invent a framework they don't have. Generate the **5 content pillars** (theme · why it matters · 5 post ideas · how it ties to the offer) into the Content pillars section.
...
- **Weekly structure + editorial calendar (deep; run via the `storytelling-strategist` lens).** Capture each day's **theme · pillar focus · objective · content angle** → `references/positioning.md → Weekly content calendar` (the owner's "I want to talk about *(pillar focus)* on *(day)* with theme *(theme)*, because *(objective)*" structure). Then generate the dated **yearly editorial plan** (5 posts/week, Mon–Fri) → `references/content-calendar.md`, so `/post` pulls the right brief by date and `/start-day` surfaces today's post. Build it on **two layers** (see `content-calendar.md.example → "How this calendar is built"`): the FIXED weekday themes give the horizontal rhythm, and a **monthly theme arc** — one umbrella idea per month, each weekday a facet of it — gives the vertical arc so the year tells a story instead of looping. **Theme + Objective are fixed by weekday, so write them ONCE into the file's legend (mirroring `positioning.md`), not per row; the table rows carry only Date · Day · Pillar focus · Topic/Angle** (Pillar focus = that month's facet). List the monthly themes in the file's header note, start the table from the next upcoming weekday (skip past dates), and leave a few `(open)` slots near the horizon rather than over-planning. If the owner has a scorecard/diagnostic that's *theirs*, capture its core axis + dimensions (they become value angles); never manufacture a generic one.
...
**4b. Deep Taste Interview → `references/about-me.md` + `references/voice-profile.md` + `references/profile.md` + `.claude/rules/voice.md` + `kk-post.md` + `kk-carousel.md`** (deep; run via the **`storytelling-strategist`** lens — PAS/9-stage story → structured `profile.md` → Story bank → fills). Offer: run now / later (`/onboard voice`) / skip. `/onboard voice` resumes here. When 4b deepens `voice.md`, treat 4a's light version as a starting draft to expand and refine in place — one `voice.md`, deepened, never a second parallel file or a clobbered 4a.
...
- **Fill the post + carousel libraries → `kk-post.md` AND `kk-carousel.md`** (format libraries whose `[bracketed directions]` are the FORMAT, never changed, and whose lines read `[xxx]` until filled). Use the captured voice + stories (the SAME story set feeds both) to replace the `[xxx]` lines with the owner's own worked example per format, leaving every `[bracketed direction]` as-is. **Follow each file's header — the "Principles for filling these formats" (kk-post) and "Carousel principles" (kk-carousel)** — as the canonical filling rules (concrete scene, real numbers, no overlap across formats, compliance Mode 1↔2, `[xxx — needs your real X]` over invention). Use `kk-post-example.md` for the *shape and density* of a filled format (its structure, never its content). Fill as many formats as the material genuinely supports — when the interview is rich, well past 10, not a token handful. Only where a format's specific lived detail is genuinely missing do you leave a clear `[xxx — needs your real X]`; the `post` / carousel skills fill only those real gaps on demand.
...
## Step 7 — Media → `config.json.media` + `references/brand-system.md`
...
- **Brand system → `references/brand-system.md`** (brief; needed by `linkedin-carousel-creator`). Confirm/edit the visual standard with the owner: colours, fonts, canvas size, and the slide-layout rotation. The file ships with sensible defaults; capture the owner's real brand where they have one, otherwise confirm the defaults.
...
## Rules
- Only write `account-profile.md`, `config.json`, `.claude/rules/*`, `kk-post.md`, `kk-carousel.md`, `references/*`. Never edit `sops/`, other skills, or `CLAUDE.md`.
